...
layout_centered(struct server_view *view) {
    ...
    if (buf_w <= 1 || buf_h <= 1) {
        wl_subsurface_set_position(view->subsurface, 0, 0);
    ...
    if (x >= 0 && y >= 0) {
        ...
        wp_viewport_set_source(view->viewport, wl_fixed_from_int(0), wl_fixed_from_int(0),
...
on_xdg_toplevel_configure(void *data, struct xdg_toplevel *xdg_toplevel, int32_t width,
    ...
    if (fullscreen) {
        int32_t render_w = ui->fullscreen_width;
        int32_t render_h = ui->fullscreen_height;

        if (render_w == 0 && render_h == 0) {
            render_w = ui->server->backend->output_width;
            render_h = ui->server->backend->output_height;
        }

        ui->render_width = (render_w > 0) ? render_w : ui->width;
        ui->render_height = (render_h > 0) ? render_h : ui->height;
    } else {
